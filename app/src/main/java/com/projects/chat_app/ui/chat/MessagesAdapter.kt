...
class MessagesAdapter(var messages:MutableList<Message>):RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return if (viewType==MessageType.Sent.value) {
            val messageViewBinding=DataBindingUtil.inflate<ItemSentMessageBinding>(LayoutInflater.from(parent.context),R.layout.item_sent_message,parent,false)
            SentViewHolder(messageViewBinding)
        } else {
            val messageViewBinding=DataBindingUtil.inflate<ItemReceivedMessageBinding>(LayoutInflater.from(parent.context),R.layout.item_received_message,parent,false)
    ...
    class ReceivedViewHolder(private val viewBinding:ItemReceivedMessageBinding):RecyclerView.ViewHolder(viewBinding.root)
    {
        fun bind(message:Message)
        {
            viewBinding.message=message
